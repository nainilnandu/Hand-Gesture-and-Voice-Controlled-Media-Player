...
const searching = document.getElementById('media-search')
...
Mousetrap.bind('p', playMusic);
Mousetrap.bind('l', pauseMusic);
// Mousetrap.bind('f', fullscreenfn);
// Mousetrap.bind('esc', closeFullscreen);

function mediaSearch(title) {
  title = title.toLowerCase()
  console.log(title);
  for (let i = 0; i < allMusic.length; i++) {
    const name = allMusic[i].name.toLowerCase()
    console.log(name);
    if (Boolean(name.match(title))) {
      searching.innerHTML = `Searching ${name}`
      console.log(title, "matched", allMusic[i].name);
      loadMusic(i + 1);
      searching.innerHTML = `Playing ${name}`
      playMusic()
      break
    }
    else console.log("unmatch");
    if (i == (allMusic.length - 1)) console.log("no result");
  }
  console.log("end");
}
